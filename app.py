...
from malware_core import MalwareDetector
...
# Initialize blockchain and malware detector
...
def upload_file():
    try:
        ...
        if file:
            ...
            try:
                logger.info(f"Starting malware detection for: {filepath}")
                result = detector.analyze_file(filepath)
                logger.info(f"Detection result: {result}")
            except Exception as e:
                logger.error(f"Error during malware detection: {str(e)}")
                logger.error(traceback.format_exc())
                return jsonify({'error': f'Error during malware detection: {str(e)}'}), 500
            ...
            try:
                ...
                logger.info("Result added to blockchain")
            except Exception as e:
...
def get_blockchain():
    try:
        ...
        return jsonify({'error': f'Error getting blockchain data: {str(e)}'}), 500

@app.route('/clear-blockchain', methods=['POST'])
def clear_blockchain():
    try:
        success = blockchain.clear_chain()
        return jsonify({'success': success})
    except Exception as e:
        logger.error(f"Error clearing blockchain: {str(e)}")
        return jsonify({'error': f'Error clearing blockchain: {str(e)}'}), 500
